    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i=0; i<files.length;i++){
            ...
            for (int w = 0; w < data.size(); w++) {
                System.out.println(data.get(w).name() + " - " + data.get(w).value());
                writer.println(data.get(w).name());
                image.drawText(data.get(w).name(), (int)Math.floor(Math.random()*x), (int) Math.floor(Math.random()*y), HersheyFont.ASTROLOGY, 20, RGBColour.RED);
            }
            writer.close();
